...
but the outcome is powerful as it allows us to simulate any number of plate appearances. I created three random dice rolls, and assigned them to specific outcomes for Babe Ruth the pitcher and batter. I use counts to keep track of every outcome, whether it's a hit, strikeout, error, out, or [fielder's choice](https://www.baseball-reference.com/bullpen/Fielder%27s_choice). The code also keeps track of singles, doubles,triples and home runs, and calculates batting average (AVG), on base percentage (OBP) and slugging (SLG).
